...
<template>
    <div class="bg-secondary border border-secondary rounded-3xl flex flex-col mb-8">
        <img :src="`/assets/projects/${image}`" alt="" class="rounded-t-3xl aspect-video object-cover">
        <div class="p-8 h-full flex flex-col justify-between">
            <div>
                <h2 class="text-2xl font-black mb-2">{{ name }}</h2>
                <p>{{ description }}</p>
            </div>
            ...
            <a v-if="gameplay" :href="gameplay" target="_blank" class="bg-red-500 border border-red-600 font-bold text-white px-4 py-2 rounded-lg block text-center mt-4 hover:bg-red-600">Gameplay</a>
